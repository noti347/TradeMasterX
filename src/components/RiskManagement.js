...
const RiskManagement = ( { positions, accountInfo } ) => {
  const calculateTotalRisk = () => {
    const totalExposure = positions.reduce((sum, position) => sum + (position.volume || 0) * 100000, 0);
    return accountInfo?.balance ? ((totalExposure / accountInfo.balance) * 100).toFixed(2) : '-';
  } ;
  ...
  return (
    <>
      ...
      <List dense>
        ...
        <ListItem>
          <ListItemText primary="Open Positions" secondary= {positions?.length || 0} />
        </ListItem>
        <ListItem>
          <ListItemText 
            primary="Total Exposure" 
            secondary= {`$$ {positions.reduce((sum, position) => sum + (position.volume || 0) * 100000, 0).toFixed(2)} `} 
          />
        </ListItem>
      </List>
    </>
